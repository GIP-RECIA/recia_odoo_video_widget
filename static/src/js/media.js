    ...
    var VideoWidget = MediaWidget.extend({
        ...
        _getVideoURLData: function (url, options) {
            ...
            const regexes = {
                ...
                pod: /^(?:(?:https?:)?\/\/)?(pod\.recia\.fr\/video|.*\.fr\/POD\/video)\/(.*)\/(\?is_iframe=true)?$/,
            };
            ...
            if (matches.youtube && matches.youtube[2].length === 11) {
                ...
                type = 'youku';
            } else if (matches.pod && matches.pod[2].length) {
                const serverUrl = matches.pod[1];
                const videoId = matches.pod[2];
                embedURL = `//${serverUrl}/${videoId}/?is_iframe=true`;
                type = 'pod';
            }
